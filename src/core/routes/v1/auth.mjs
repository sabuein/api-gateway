...
router.post("/refresh", (req, res, next) => {
    try {
        ...
        res.cookie("jwtRefresh", token, cookieOptions());
...
router.post("/access", async (req, res, next) => {
    try {
        if (req.signedCookies.refresh) {
            ...
            res.cookie("jwtAccess", token, cookieOptions("access"));
...
router.post("/login", validate(userSchema), async (req, res, next) => {
    try {
        ...
        try {
            ...
            const accessToken = access(req.user);
            const refreshToken = refresh(req.user);
            res.cookie("jwtAccess", accessToken, cookieOptions("access"));
...
router.all("/logout", (req, res, next) => {
    try {
        ...
        res.clearCookie("jwtAccess", cookieOptions("access"));
        res.clearCookie("jwtRefresh", cookieOptions());

        res.status(200).json({ message: "Signed out successfully." });
        res.status(204).sendFile(
