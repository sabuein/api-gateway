...
import { getLocationAsync } from "web";
// import "https://api.tomtom.com/maps-sdk-for-web/cdn/6.x/6.25.0/maps/maps-web.min.js";
...
/** Load TomTom's API key from the server. */
const loadTomTomApiKey = async () => await apiCall({
    endpoint: "config/tomtom",
    method: "POST",
});
...
const initializeTomTomMap = async () => {
    ...
    const { apiKey } = await loadTomTomApiKey();
    if (!apiKey) addMessage("No API public key returned from the server for TomTom. Please check and try again.");

    const myMap = tt.map({
        key: window.atob(apiKey),
        container: "map",
...
const getTomTomCurrentAddress = async () => {
    try {
        const { apiKey } = await loadTomTomApiKey();
        if (!apiKey) addMessage("No API public key returned from the server for TomTom. Please check and try again.");
        
        const position = await getLocation(position);
        const coords = position.coords;
        const address = await tt.services
            .reverseGeocode({
                key: apiKey,
        ...
        if (!!address) currentAddressCallback(address);
    } catch (error) {
        console.error("Error in getTomTomCurrentAddress:", error);
        throw error; // Propagate the error to the caller
    }
};

const currentAddressCallback = (response) => {
...
const getScreenShot = async (zoom) => {
    try {
        const { apiKey } = await loadTomTomApiKey();
        if (!apiKey) addMessage("No API public key returned from the server for TomTom. Please check and try again.");
        ...
        const url = await tt.services.staticImage({
            key: apiKey,
            zoom: zoom,
...
export { initializeTomTomMap, getScreenShot };
